...
class data_img:
    def __init__(self,img_path, bits_per_char=8, headers=None):
        ...
        if(headers == None):
        	...
        	} #used for retreving files  
        else:
        	self.headers = headers
        self.pil_image = self.load_image(img_path)
        self.pixels = self.pil_image.load()
        self.width, self.height = self.pil_image.size    

    def load_image(self,img_path):
        def get_pil_image(uri):
            image = img.open(io.BytesIO(url.urlopen(url.Request(uri, headers=self.headers)).read()))
            return image
    ...
    def encode_file(self,filename, bits_per_px=3, bits_per_char=8, start=0):
        data = ''
        print("loading and coping file this could take a while")
        try:
            f = open(filename, 'rb')
            split_at_question_mark = False
        except OSError:
            f = io.BytesIO(url.urlopen(url.Request(filename, headers=self.headers)).read())
            split_at_question_mark = True

        filename = filename.split('/')[-1]
        if(split_at_question_mark):
...
if(__name__ == '__main__'):
    ...
    f.encode_file("https://www.washingtonpost.com/rf/image_1484w/2010-2019/WashingtonPost/2018/04/11/Investigative/Images/DogAuction_SG28.JPG?t=20170517").save("file.png")
    print('\n'*3)
    a = data_img("file.png").decode_file()
    print(a)